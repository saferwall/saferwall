...
var filepathScanTest = []filePathTest{
	{"../../../test/multiav/eicar.com", Result{Infected: true, Output: "Eicar test file"}},
}
...
func TestGetProgramVersion(t *testing.T) {
	...
	if len(l) == 0 {
		t.Fatalf("Program version was incorrect, got: %s, want something similar to: 4.5.13", version)
	}
}
