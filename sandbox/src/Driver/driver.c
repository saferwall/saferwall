...

{
    ...
    if (!NT_SUCCESS(ntStatus))
    {
        if (ntStatus == STATUS_ACCESS_DENIED)
        {
            ...
            LOG_ERROR(
                "PsSetCreateProcessNotifyRoutineEx() failed; ensure /INTEGRITYCHECK linker flag was used during linking");
        }
        else
        {
        ...
        // DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "ObCallbackTest: DriverEntry:
